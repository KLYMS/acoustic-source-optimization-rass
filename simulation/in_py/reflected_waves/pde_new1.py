# in this code which is updated version of pde_solve.py which uses 
# classes which are now used as an definitions (func's) 
    ...
    if Ideal_Conditions:
        ...
        if With_wind: 
            if With_spline:  
                ...
                wind_speed = avg_data(height_data,wind_speed_data,z)
                wind_dir = avg_data(height_data,wind_direction_data,z)
                dvx_dz = slope(height_data,wind_speed_data,z) * np.cos(np.deg2rad(wind_dir)) -(
                        np.sin(np.deg2rad(wind_dir)) * slope(height_data,wind_direction_data,z) * wind_speed
                ) 
                dvy_dz = slope(height_data,wind_speed_data,z) * np.sin(np.deg2rad(wind_dir)) + (
                        np.cos(np.deg2rad(wind_dir)) *slope(height_data,wind_direction_data,z) * wind_speed
                )
        else:
            wind_speed = 0
            wind_dir = 0
...
def integrate_(y,km,h, phi):
    ...
    for i in range(1,steps+1):
        ...
        # distance_traveled = np.sqrt(y_values[i][0]**2 + y_values[i][1]**2 + y_values[i][2]**2)
        # if distance_traveled > km * 1000:
...
if not Ideal_Conditions:
    ...
    data = pd.read_csv('/home/murali/Documents/rass/simulation/in_py/reflected_waves/radiosonade_ans.csv')
...
elif(With_wind and not Ideal_Conditions):

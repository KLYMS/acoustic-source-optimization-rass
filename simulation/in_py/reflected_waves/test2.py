...
# Set up logging
logging.basicConfig(filename='ray_tracing_progress_test2.log' , level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def main():
    ...
    global height_data, temperature_data, wind_x_data, wind_y_data, temperature_spline, wind_x_spline, wind_y_spline

    if not Ideal_Conditions:
        logger.info('Reading the Radiosonde data')
        ...
        if With_spline:
            logger.info("Creating Splines from the data")
    ...
    theta_min = - np.pi / 2
    theta_max = np.pi / 2
    theta_step = np.pi / 300

    phi_min = np.deg2rad(0)
    phi_max = np.deg2rad(180)
    phi_step = np.deg2rad(20)
    ...
    with open(w_filename, 'w') as csvfile:
        ...
        with ProcessPoolExecutor() as executor:
            futures = [executor.submit(process_params, params) for params in params_list]
            ...
            for future in as_completed(futures):
                ...
                logging.info(f"Completed {completed} out of {total} combinations ({progress:.2f}%)")

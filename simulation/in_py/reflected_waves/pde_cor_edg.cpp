/*
 * this is the translated version of the pde_cor_edg.py code 
 */
...
tk::spline temperature_spline, wind_x_spline, wind_y_spline;
...
int main() {
    ...
    if (!Ideal_Conditions) {
        ...
        string filename = "/home/murali/Documents/rass/data/radiosonde_grouped_data.csv";
        read_csv(columns, filename);
        ...
        if (With_spline){
            ...
            temperature_spline.set_boundary(tk::spline::first_deriv,0.0,
                                            tk::spline::first_deriv,0.0);
            wind_x_spline.set_boundary(tk::spline::first_deriv,0.0,
                                        tk::spline::first_deriv,0.0);
            wind_y_spline.set_boundary(tk::spline::first_deriv,0.0,
                                        tk::spline::first_deriv,0.0);
            

            temperature_spline.set_points(height_data, temperature_data, tk::spline::cspline);
            wind_x_spline.set_points(height_data, wind_x_data, tk::spline::cspline);
            wind_y_spline.set_points(height_data, wind_y_data, tk::spline::cspline);
        }
    ...
    if (With_temperature && With_wind) {
        w_filename = "wTemp_wWind_cpp.csv";
    } else if (With_temperature && !Ideal_Conditions) {
...
//         cerr << "Empty file or unable to read header" << endl;
//         file.close();
//         return 1;
//     }
...
//         while (getline(lineStream, cell, ',')) {
//             try {
//                 // Convert cell string to double and add to row
//                 row.push_back(stod(cell));
//             } catch (const invalid_argument& e) {
//                 cerr << "Invalid input in CSV: " << cell << endl;
//             } catch (const std::out_of_range& e) {
//                 cerr << "Input out of range in CSV: " << cell << endl;
//             }
//         }
        
//         // Add the row to the columns
...
// double avg_data(const vector<double>& height_data, const vector<double>& data, double target_height) {
//     vector<double>::const_iterator it = lower_bound(height_data.begin(), height_data.end(), target_height);

//     if (it != height_data.end() && *it == target_height) {
//         return data[it - height_data.begin()];
//     } else {
//         vector<double>::difference_type insert_pos = distance(height_data.begin(), it);
//         if (insert_pos == 0) {
//             return data[0];
//         } else if (insert_pos == height_data.size()) {
//             return data.back();
//         } else {
//             double h1 = height_data[insert_pos - 1];
//             double h2 = height_data[insert_pos];
//             double t1 = data[insert_pos - 1];
//             double t2 = data[insert_pos];
//             double slope_ = (t2 - t1) / (h2 - h1);
//             double interpolated_data = t1 + ((target_height - h1) * slope_);
...
//     if (it != height_data.end() && *it == target_height) {
//         int index = it - height_data.begin();
...
//         }
//     } else {
//         auto insert_pos = distance(height_data.begin(), it);
//         if (insert_pos >= height_data.size() - 1) {
//             int index = height_data.size() - 1;
//             return backward_calc(index, height_data, data);
//         }
...
double avg_data(const vector<double>& height_data, const vector<double>& data, double target_height ){
    ...
    if (it != height_data.end()) {
        ...
        if (index == 0) {
            ...
            interpolated_data = data.back();
        } else {
            double h1 = height_data[index - 1];
            double h2 = height_data[index];
            double t1 = data[index - 1];
            double t2 = data[index];
            double slope_ =  (t2 - t1) / (h2 - h1);
    ...
    if (Ideal_Conditions) {
        ...
        if (With_temperature) {
            if (With_spline) {
                ...
                if (temp <= 0) temp = 292.804;
                dc_dz = sqrt(Gamma * R_air / temp) * 0.5 * temperature_spline.deriv(1,z);
            }else{
        ...
        if (With_wind) {
            if (With_spline){
                v_x = wind_x_spline(z);
                v_y = wind_y_spline(z);
                dvx_dz = wind_x_spline.deriv(1,z);
                dvy_dz = wind_y_spline.deriv(1,z);
            }else{

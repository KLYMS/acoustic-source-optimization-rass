...
int main() {
    ...
    ofstream output_file("backscatter_coordinates_i_s_bCS.csv");
    ...
    if (data_filtering) filterData(x, y, z, time_slices, offset_dist);
    log_message("Done Data Filtering\nNow fitting the Surface having rho: " + to_string(rho));
...
void log_message(const string &message) {
    ofstream log_file("backscattering_progress_test.log", ios_base::app);
...
void fitSplineSurface(alglib::spline2dinterpolant& s, const vector<double>& x, const vector<double>& y, const vector<double>& z) {
    ...
    if (nx < 2 || ny < 2) {
        log_message("Insufficient data points for spline fitting: nx = " + to_string(nx) + ", ny = " + to_string(ny));
        throw alglib::ap_error("Insufficient data points");
    }

    // Create real_1d_array objects for x, y, and z values
    alglib::real_1d_array x_array, y_array, f_array;
    x_array.setlength(nx);
    y_array.setlength(ny);
    f_array.setlength(nx * ny);
    ...
    // Fill in f_array with z values in the appropriate order
    for (int j = 0; j < ny; ++j) {
        for (int i = 0; i < nx; ++i) {
            f_array[j * nx + i] = z[j * nx + i];
        }
    }

    alglib::spline2dbuildbicubicv(x_array, nx, y_array, ny, f_array, 1, s);

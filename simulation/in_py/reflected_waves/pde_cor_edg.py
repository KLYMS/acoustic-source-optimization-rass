# this code is updated version of pde_new2.py
# updated the corner and edge acoustic sources of an NxN size of an anttena array 
...
if not Ideal_Conditions:
    ...
    if With_spline:
        ...
        temperature_spline = CubicSpline(height_data, temperature_data,bc_type='not-a-knot' ) 
        wind_speed_spline = CubicSpline(height_data, wind_speed_data, bc_type='not-a-knot')
        wind_direction_spline = CubicSpline(height_data, wind_direction_data, bc_type='not-a-knot')
        wind_x_spline = CubicSpline(height_data,wind_x_data,bc_type='not-a-knot')
        wind_y_spline = CubicSpline(height_data,wind_y_data,bc_type='not-a-knot')
...
with open(w_filename, 'w') as csvfile:
    ...
    for _ , arr in enumerate(corner_val):
        for phi in drange(phi_min,phi_max,phi_step):

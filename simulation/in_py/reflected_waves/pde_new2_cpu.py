...
def integrate_(theta):
    y = [0, 0, 0, theta]
    km = height_data.max() / 1000 if not Ideal_Conditions else 30
    h = 1.5
    phi = np.deg2rad(90)
    t0 = 0
    ...
    return np.column_stack([y_values, t_values])
...
if not Ideal_Conditions:
    ...
    data = pd.read_csv('radiosonade_ans_grp.csv')
...
if __name__ == '__main__':
    with Pool(cpu_count()) as pool:
        results = pool.map(integrate_, drange(theta_min, theta_max, theta_step))

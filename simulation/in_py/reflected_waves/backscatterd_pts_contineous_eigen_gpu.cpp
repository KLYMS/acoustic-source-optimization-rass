...
// CUDA kernel for finding normal at a point
__global__ void findNormalAtPointKernel(double* x, double* y, double* z, double* dzdx, double* dzdy, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
        double dzdx_val, dzdy_val, z_eval;
        // Evaluate spline and calculate derivatives
        alglib::spline2ddiff(s, x[idx], y[idx], z_eval, dzdx_val, dzdy_val);
        // Store results
        dzdx[idx] = dzdx_val;
        dzdy[idx] = dzdy_val;
    }
...
int main() {
    ...
    for (size_t idx = 0; idx < time_slices.size(); ++idx) {
        if (time_slices[idx] != 0) {
            ...
            int n = x.size();
            double *d_x, *d_y, *d_z, *d_dzdx, *d_dzdy;
            cudaMallocManaged(&d_x, n * sizeof(double));
            cudaMallocManaged(&d_y, n * sizeof(double));
            cudaMallocManaged(&d_z, n * sizeof(double));
            cudaMallocManaged(&d_dzdx, n * sizeof(double));
            cudaMallocManaged(&d_dzdy, n * sizeof(double));
            ...
            cudaMemcpy(d_z, z.data(), n * sizeof(double), cudaMemcpyHostToDevice);

            // Launch the kernel to find normals
            int blockSize = 256;
            int numBlocks = (n + blockSize - 1) / blockSize;
            findNormalAtPointKernel<<<numBlocks, blockSize>>>(d_x, d_y, d_z, d_dzdx, d_dzdy, n);
            cudaDeviceSynchronize(); // Ensure kernel execution is complete

            // Copy results back to the host
            vector<double> dzdx(n), dzdy(n);
            cudaMemcpy(dzdx.data(), d_dzdx, n * sizeof(double), cudaMemcpyDeviceToHost);
            cudaMemcpy(dzdy.data(), d_dzdy, n * sizeof(double), cudaMemcpyDeviceToHost);
            ...
            for (size_t idx_x = 0; idx_x < x.size(); ++idx_x) {
                ...
                // Use the computed dzdx and dzdy to find the landing point

...
vector<double> wind_data;

int read_csv(vector<double>& column1, vector<double>& column2, vector<double>& column3){

    ifstream file("wind_radio_data.csv");
    if (!file.is_open()) {
...
double avg_data(double target_height,vector<double>& height_data, vector<double>& data ){
...
class ODESystem {
    ...
    private:
        ...
        /*     /1* if(With_temperature){ *1/ */
        /*     /1*     if (z <= Tropopause_Height) return 300 + (Troposphere_Lapse_Rate / 1000) * z; *1/ */ 
        /*     /1*     else if(z > Tropopause_Height && z<= Stratopause_Height ) return temperature(Tropopause_Height) + (Stratosphere_Lapse_Rate / 1000) * (z - Tropopause_Height); *1/ */ 
        /*     /1*     else return temperature(Stratopause_Height); *1/ */
        /*     /1* }else return 0; *1/ */
        /*     double temp = */     
        /*     double slp = slope */
        /*     return slp; */
        ...
        double wind_speed(double z){
            if(With_wind){
                double ws = avg_data(z,height_data,temp_data);
                return ws;
            }
            else return 0;
        }
        ...
        double c_s(double z){
            if(With_temperature){
                double t = avg_data(z,height_data,temp_data);
        ...
        double partial_cs_z(double z){
            if(With_temperature){
                double n = sqrt((Gamma * R_gas)/(4 * avg_data(z,height_data,temp_data)));
        ...
        double partial_vx_z(double z){
            if(With_wind){
                return slope(height_data,wind_data,z);
            }
            else return 0;
        }
...
int main(){
    ...
    /* string t_ = "temp_radio_data.csv"; */
    /* string w_ = "wind_radio_data.csv"; */
    /* read_csv(height_data,wind_data, w_); */
    /* read_csv(height_data,temp_data, t_); */

    read_csv(height_data,temp_data,wind_data);

...
// updated version of the pde_cor_edg.cpp which uses the ALGLIB library for the Spline Interpolation 
...
int main() {
    ...
    if (!Ideal_Conditions) {
        ...
        string filename = "/home/murali/Documents/rass/automation/gen_data/F2021041817S9002660_cleaned.csv";
        ...
        if (With_spline){
            ...
            // real_1d_array height_array, temperature_array, wind_x_array, wind_y_array;

            // height_array.setcontent(height_data.size(), height_data.data());
            // temperature_array.setcontent(temperature_data.size(), temperature_data.data());
            // wind_x_array.setcontent(wind_x_data.size(), wind_x_data.data());
            // wind_y_array.setcontent(wind_y_data.size(), wind_y_data.data());

            // spline1dbuildcubic(height_array, temperature_array, temperature_spline);
            // spline1dbuildcubic(height_array, wind_x_array, wind_x_spline);
            // spline1dbuildcubic(height_array, wind_y_array, wind_y_spline);
    ...
    if (With_temperature && With_wind) {
        w_filename = "/home/murali/Documents/rass/automation/ F2021040317S8050069_processed_wf.csv";
    } else if (With_temperature && !Ideal_Conditions) {
        w_filename = "wTemp.csv";
    } else if (With_temperature && Ideal_Conditions) {
        w_filename = "ideal_wTemp.csv";
    } else {
        w_filename = "ideal_cnds_0.csv";
    }
